...
main(int argc, char* argv[])
{
    ...
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Your code goes here
    NodeContainer c, c_router, c_hosts;
    ...
    for (int i = 0; i < (int)j_nodes.size(); i++)
    {
        ...
        if (j_nodes[i]["type"].asString() == router_identifier)
        {
            routers[j_nodes[i]["id"].asString()] = c_router.Get(routers.size());
        }
        else if (j_nodes[i]["type"].asString() == host_identifier)
        {
            hosts[j_nodes[i]["id"].asString()] = c_hosts.Get(hosts.size());
        }
        //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    }
    ...
    std::unordered_map<std::string, std::string> host_gateways;
    for (int i = 0; i < (int)j_links.size(); i++)
    {
        //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Your code goes here
        std::string source = j_links[i]["source"].asString();
        std::string target = j_links[i]["target"].asString();
        edges[source + target].Add(node_map[source]);
        edges[source + target].Add(node_map[target]);
        if (hosts.find(source) != hosts.end())
        {
            host_gateways[source] = target;
        }
        if (hosts.find(target) != hosts.end())
        {
            host_gateways[target] = source;
        }
        //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    }
     ...
     */
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Your code goes here
    for (auto it = host_gateways.begin(); it != host_gateways.end(); it++)
    {
        ripRouting.ExcludeInterface(node_map[it->first], iface_nums[it->first][it->second]);
    }
    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
     ...
     */
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Your code goes here
    for (auto it = iface_nums.begin(); it != iface_nums.end(); ++it)
    {
        std::string nodeName = it->first;
        Ptr<Node> node = node_map[nodeName];

        for (auto iface : it->second)
        {
            std::string connectedNode = iface.first;
            int ifaceIndex = iface.second;

            int metric = 1;
            ripRouting.SetInterfaceMetric(node, ifaceIndex, metric);
        }
    }
    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    /*
    ...
    for (auto it = gateways.begin(); it != gateways.end(); it++)
    {
        ...
        Ipv4Address gatewayAddress = ipv4Gateway->GetAddress(iface_nums[it->second][it->first], 0);

        Ptr<Ipv4GlobalRouting> globalRouting = ipv4Host->GetObject<Ipv4GlobalRouting>();
        if (globalRouting)
        {
            globalRouting->AddHostRouteTo(Ipv4Address("0.0.0.0"),
                                          gatewayAddress,
                                          iface_nums[it->first][it->second]);
            //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        }
        ...
        for (int i = 0; i < (int)j_flows.size(); i++)
        {
            ...
            if (j_flows[i]["Type"].asString() == "Ping")
            {
                ...
                ping.SetAttribute("StartTime",
                                  TimeValue(Seconds(j_flows[i]["StartTime"].asDouble())));
                ping.SetAttribute("StopTime",
                                  TimeValue(Seconds(j_flows[i]["StopTime"].asDouble())));
                ApplicationContainer app = ping.Install(node_map[u]);

                AsciiTraceHelper asciiTraceHelper;
                std::stringstream fname_rtt;
                fname_rtt << "output/rip/" << key.str() << ".rtt";
                Ptr<OutputStreamWrapper> streamRtt =
                    asciiTraceHelper.CreateFileStream(fname_rtt.str());
                app.Get(0)->TraceConnectWithoutContext("Rtt",
                                                       MakeBoundCallback(&TraceRtt, streamRtt));
                //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            }
        }
